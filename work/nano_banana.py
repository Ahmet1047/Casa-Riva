...
DEFAULT_PROMPT = (
    ...
    "Both shoes are ONE single identical model and must match each other exactly in shape and details; "
    "do NOT warp, stretch, deform or redraw them and do NOT make two different shoes. "
    "If several shoe references are given, the LEFT (top-down) shoe matches the top-down reference and "
    "the RIGHT (side) shoe matches the side-profile reference, but both stay the same identical model. "
...
RECOLOR_PROMPT = (
    ...
    # Geometrie-/Identitaets-Lock (P8): nur die Oberflaeche neu faerben, nie neu zeichnen.
    "Both shoes are ONE single identical sneaker model shown from two angles; they must match each "
    "other exactly in shape, proportions and every detail. Preserve the exact geometry of the shoes "
    "from the first image and only repaint their surfaces. Do NOT warp, stretch, melt, deform or "
    "redraw the shoes, do NOT turn them into two different shoes, keep clean realistic undistorted "
    "sneaker proportions. "
